...
import { ArrowLeft, Eye, EyeOff, Music, Trash2, RotateCcw } from "lucide-react"
...
export default function HiddenScoresPage() {
  const { user: authUser } = useAuth()
  const [hiddenScores, setHiddenScores] = React.useState<HiddenScore[]>([])
          ...
          try {
            ...
            if (scoreDoc.exists()) {
              ...
              scoresData.push({
                ...
                thumbnailUrl: data.flatid ? `https://flat.io/api/v2/scores/${data.flatid}/revisions/last/thumbnail.png?width=300&height=400` : undefined,
                sharing: data.sharing || 'private',
                createdAt: data.created ? new Date(data.created.toDate()).toLocaleDateString() : undefined
              })
            }
          } catch (error) {
  ...
  const handleUnhideScore = (scoreId: string) => {
    try {
      ...
      toast({
        title: "Success",
        description: "Score has been unhidden and will appear in the community again",
        type: "success"
      })
    } catch (error) {
          ...
          <div className="container max-w-4xl mx-auto px-4">
              ...
              <div className="flex items-center justify-between">
                ...
                {hiddenScores.length > 0 && (
                  <Button
                    ...
                    variant="outline"
                    className="text-[#4A1D2C] dark:text-[#8A3D4C] border-[#4A1D2C] dark:border-[#8A3D4C] hover:bg-[#4A1D2C] hover:text-white dark:hover:bg-[#8A3D4C]"
                  >
            ...
            {isLoading ? (
              <div className="flex items-center justify-center py-12">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#4A1D2C] dark:border-[#8A3D4C]"></div>
              </div>
            ) : hiddenScores.length === 0 ? (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                ...
                <Link href="/community">
                  <Button className="bg-[#4A1D2C] hover:bg-[#3A1520] dark:bg-[#8A3D4C] dark:hover:bg-[#6A2D3C] text-white">
                    Browse Community
                  </Button>
                </Link>
              </motion.div>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            ...
                            size="sm"
                            variant="ghost"
                            className="text-[#4A1D2C] dark:text-[#8A3D4C] hover:text-white hover:bg-[#4A1D2C] dark:hover:bg-[#8A3D4C]"
                          >
