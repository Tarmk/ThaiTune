...
export default function NewScorePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!flatId.trim()) throw new Error('Flat.io ID is required')
          ...
          <Card className="max-w-2xl">
            <CardContent className="p-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="flatId">Flat.io Score ID</Label>
                  <Input
                    id="flatId"
                    value={flatId}
                    onChange={(e) => setFlatId(e.target.value)}
                    placeholder="Enter Flat.io score ID"
                    required
                  />
                  <p className="text-sm text-gray-500">
                    You can find the score ID in the URL of your Flat.io score
                  </p>
                </div>
                <Button
                  type="submit"
